...
public class ThermPlotPanel extends VerticalPanel
{
	...
	private double[] min = {0.0, 30.0, 70.0};
	private double[] max = {30.0, 70.0, 100};
	private String[] color = {"#0000ff", "#00ff00", "#ff0000"};
	...
	public void setMax(double[] max) {for (int ii = 0; ii < 3; ++ii) this.max[ii] = max[ii];}
	public void setColor(String[] color) {for (int ii = 0; ii < 3; ++ii) this.color[ii] = color[ii];}
	...
	public ThermPlotPanel(EntryPointApp entryPointApp)
	{
		loaded = false;
	...
	public void draw() 
	{
		double[] tempVal = {min[0],min[0],min[0]};
		...
		for (int ii = 0; ii < 3; ++ii)
		{
			...
			else
			{
				...
				{
					tempVal[ii] = value;
					haxisTextStyle.setColor(color[ii]);
				}
			}
