package se.esss.litterbox.its.dashboardgwt.client.mqttdata;
...
//				mqttData.entryPointApp.getSetupApp().getStatusTextArea().addStatus(mqttData.topic + " " + result[ii][0] + " " +  result[ii][1]);
	...
	private static class JsonDataTimer extends Timer
	{
		...
		public void run() 
		{
			mqttData.entryPointApp.mqttService.getJsonArray(mqttData.topic, false, null, new GetJsonDataAsyncCallback(mqttData));
		}
	...
	private static class ByteDataTimer extends Timer
	{
		...
		public void run() 
		{
			mqttData.entryPointApp.mqttService.getMessage(mqttData.topic, false, null, new GetByteDataAsyncCallback(mqttData));
		}
